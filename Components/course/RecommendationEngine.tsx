...
export default function RecommendationEngine({ currentCourseId, currentCategory }) {
  ...
  const getRecommendations = () => {
    ...
    courseViews.forEach(view => {
      if (view.course_category) {
        categoryScores[view.course_category] = (categoryScores[view.course_category] || 0) + view.view_count;
      }
    });

    // Boost current category
    if (currentCategory) {
      categoryScores[currentCategory] = (categoryScores[currentCategory] || 0) + 5;
    }
    ...
    const scoredCourses = allCourses
      ...
      .map(course => ({
        ...course,
        score: (categoryScores[course.category] || 0) + (course.is_popular ? 3 : 0)
      }))
